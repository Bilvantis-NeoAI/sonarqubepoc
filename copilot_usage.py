...
# Shared stdout emission helpers — see python/transport/streaming.py.
# Extracted from this file so both copilot_client.py and agent_client.py write the
# same `data: ...` envelope without drift.
from transport.streaming import (
    emit_delta_chunk,
    emit_error_message,
    emit_final_message,
    stream_sse_from_response,
)
...
def generate_response(prompt, sessionToken_b64, stream: bool = True, checkSessionExpiry=False, access_token=''):
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            # Surface the real Copilot error body so the webview shows the actual cause
            # (400 "model not found", rate limits, schema rejections) instead of a bare code.
            emit_error_message(response.status_code, response.text)
            return

        if stream:
            stream_sse_from_response(response)
        else:
            try:
                rj = response.json()
                emit_final_message(rj['choices'][0]['message']['content'])
            except Exception as e:
                logger.error(f"Failed to parse non-streaming response: {e}")
                emit_delta_chunk('Error: Failed to parse response from server')
